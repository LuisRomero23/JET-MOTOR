...
bool busqueda (char serial[10]){
	bool v = false; //esta funcion verifica que no haya seriales repetidas en el sistema
	FILE *busca;
...
void incluir(){
	char g[2];
	cout<<"\nIngrese la serial del vehiculo que quiere añadir al sistema: "; gets(k.serial);
	if(busqueda(k.serial)==true){
		...
		if (strcmp(k.g,"si")==0){
			...
			for (int i = 0 ; i < k.nrogarant ; i++){
				cout<<"\nIngrese el concepto de su garantia #" <<i+1<<": "; gets(k.garant[i].concepto[i]);
				cout<<"\nIngrese el porcentaje del precio cubierto de la garantia #" <<i+1<<": "; cin>>k.garant[i].porcentaje[i];
			}
		} else {
			k.nrogarant = 0;
		}
...
void consultar(){
	...
	if(busqueda(serialcomp)==true){ //if que busca si ambas seriales son iguales y muestre sus datos 
		...
		if (k.nrogarant <=0){ //if para validar si tiene garantias o no, si no tiene, no se muestra la info de las garantias
		...
		cout<<"\nNo se encontro el vehiculo con esta serial.";
	}
}
